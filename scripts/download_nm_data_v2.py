...
def get_county_boundaries_from_census():
    ...
    print(f"Downloading US counties from Census...")
    ...
    counties_data = [
        {
            "name": "Lea",
            "countyfp": "025",
            "geometry": box(-103.6, 32.5, -103.0, 33.2)
        },
        {
            "name": "Roosevelt",
            "countyfp": "041",
            "geometry": box(-103.8, 33.5, -103.0, 34.8)
        },
        {
            "name": "Curry",
            "countyfp": "009",
            "geometry": box(-103.6, 34.2, -103.0, 35.0)
        },
    ]
...
def create_synthetic_fields(counties_gdf, n_fields=200):
    ...
    print(f"Size range: {top_fields['area_acres'].min():.1f} - {top_fields['area_acres'].max():.1f} acres")
...
def download_soil_data(fields_gdf):
    ...
    try:
        ...
        sys.path.insert(0, str(Path(__file__).parent.parent / "skills" / "vendor" / "ag-skills" / "ssurgo-soil" / "src"))
        import ssurgo_soil
...
def download_weather_data(fields_gdf):
    ...
    if all_weather:
        ...
        # Convert -999 to NaN
        for col in ['T2M', 'T2M_MAX', 'T2M_MIN', 'PRECTOTCORR', 'ALLSKY_SFC_SW_DWN', 'RH2M', 'WS10M']:
