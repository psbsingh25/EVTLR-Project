...
def get_dominant_soil_per_field(soil_df):
    ...
    numeric_cols = ['om_r', 'ph1to1h2o_r', 'awc_r', 'claytotal_r', 
...
def create_map_with_soil(fields_gdf, output_name, title, corn_only=False):
    ...
    m = folium.Map(
        ...
        zoom_start=9 if not corn_only else 10,
        tiles="OpenStreetMap"
    )
...
def main():
    ...
    corn_fields = merged[merged['field_id'].isin(
        pd.read_csv(OUTPUT_DIR / "soil_summary_by_field.csv")['field_id']
    ...
    if len(corn_fields) > 0:
        ...
        corn_gdf = merged[merged['field_id'].isin(
            ['NM_FIELD_188', 'NM_FIELD_025', 'NM_FIELD_130', 'NM_FIELD_078', 'NM_FIELD_065']
        )]
